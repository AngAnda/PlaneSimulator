...
public:
	void ProcessKeyboard(int direction, float deltaTime)
	{
		...
		if (direction == GLFW_KEY_D)
			position += right * velocity;
		if (direction == GLFW_KEY_PAGE_UP)
			position += glm::vec3(0.0f, velocity, 0.0f);
		if (direction == GLFW_KEY_PAGE_DOWN)
			position -= glm::vec3(0.0f, velocity, 0.0f);
		UpdateCameraVectors();
	}
	...
	{
		float xChange = (xOffset - lastX) * mouseSensitivity;
		float yChange = (lastY - yOffset) * mouseSensitivity;
		...
		// Constrângerea unghiului pitch pentru a evita fliparea camerei
		if (pitch > 89.0f)
			pitch = 89.0f;
		if (pitch < -89.0f)
			pitch = -89.0f;
...
protected:
	const float cameraSpeedFactor = 400.5f;
...
void Initialize()
{
	...
	// Create camera
	pCamera = new Camera(SCR_WIDTH, SCR_HEIGHT, glm::vec3(0.5, 0.5, 30));
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_W, deltaTime);
	}
	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_S, deltaTime);
	}
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_A, deltaTime);
	}
	if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_D, deltaTime);
	}
	if (glfwGetKey(window, GLFW_KEY_PAGE_UP) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_PAGE_UP, deltaTime);
	}
	if (glfwGetKey(window, GLFW_KEY_PAGE_DOWN) == GLFW_PRESS) {
		pCamera->ProcessKeyboard(GLFW_KEY_PAGE_DOWN, deltaTime);
	}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	//pCamera->ProcessMouseControl((float)xpos, (float)ypos); 
	//ProcessMouseControl((float)xpos, (float)ypos);
	pCamera->ProcessMouseMovement((float)xpos, (float)ypos);
}
